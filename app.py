...
st.caption("内部データ + 外部情報 → 課題リスト → 提案アイデア → チェック → 提案スライド文章")

# ---------- Step 1: データ入力 ----------
with st.container():
    st.markdown('<div class="step-card"><div class="step-title">Step 1. データ入力</div>', unsafe_allow_html=True)
    uploaded_internal = st.file_uploader("内部データ (CSV/Excel/TXT)", type=["csv","xls","xlsx","txt"])
    uploaded_external = st.file_uploader("外部資料 (PDF)", type=["pdf"])
    ...
    if uploaded_external:
        external_text = data_io.load_file(uploaded_external)
    st.markdown('</div>', unsafe_allow_html=True)

# ---------- Step 2: 会社データまとめ ----------
with st.container():
    st.markdown('<div class="step-card"><div class="step-title">Step 2. 内部情報</div>', unsafe_allow_html=True)
    if st.button("内部情報要約", disabled=not bool(internal_text)):
        with st.spinner("要約中..."):
...
# ---------- Step 3: 検索 ----------
with st.container():
    st.markdown('<div class="step-card"><div class="step-title">Step 3. 検索</div>', unsafe_allow_html=True)
    if st.button("検索クエリを生成", disabled=not bool(st.session_state.internal_summary)):
    ...
    if st.session_state.queries:
        ...
        for idx, q in enumerate(st.session_state.queries):
            ...
            txt = cols[1].text_input("クエリ", value=q, key=f"edit_q_{idx}")
            if use and txt.strip():
                edited_queries.append(txt.strip())

        col1, col2 = st.columns([0.5, 0.5])
        with col1:
            if st.button("選択したクエリで検索実行", disabled=len(edited_queries) == 0):
        ...
        with col2:
            if st.button("検索せず内部データのみで進める"):
                st.session_state.search_results = {
                    "cards": [],
                    "summary": st.session_state.external_text or "",
                }
                st.session_state.executed_queries = ["検索なし（内部情報のみ）"]
                reset_downstream("issues", "proposals", "judge", "slides")

    if st.session_state.search_results:
        st.markdown("**実行クエリ:** " + ", ".join(st.session_state.executed_queries))
        for card in st.session_state.search_results["cards"]:
            st.markdown(f"- [{card['title']}]({card['url']}) ({card['source']})")
        st.markdown("**要点まとめ:**")
        st.write(st.session_state.search_results["summary"] or "_（外部要約なし）_")
    st.markdown('</div>', unsafe_allow_html=True)
...
with st.container():
    ...
    st.session_state.proposal_category = st.radio(
        "カテゴリを選択",
        ["保守", "拡大", "撤退", "すべて"],
        index=["保守", "拡大", "撤退", "すべて"].index(st.session_state.proposal_category),
    )
...
# ---------- Step 6: チェック ----------
with st.container():
    st.markdown('<div class="step-card"><div class="step-title">Step 6. チェック（矛盾確認）</div>', unsafe_allow_html=True)
    if st.button("チェックを実行", disabled=not bool(st.session_state.proposals)):
        with st.spinner("Judge実行中..."):
            st.session_state.judge = llm_utils.judge_consistency(
